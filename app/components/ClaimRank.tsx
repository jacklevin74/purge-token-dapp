...
export const ClaimRank: FC = () => {
  ...
  const handleBatchClaim = useCallback(async () => {
    ...
    try {
      ...
      // Fire all transactions concurrently
      const { blockhash } = await conn.getLatestBlockhash('confirmed');

      const txPromises = Array.from({ length: actualBatch }, async (_, i) => {
      ...
      const batchResultsList: BatchResult[] = results.map((r, i) => {
        if (r.status === 'fulfilled') return r.value;
        return { slot: startSlot + i, success: false, error: String(r.reason) };
      });

      setBatchResults(batchResultsList);
          ...
          >
            {loading ? (
              <span className="flex items-center justify-center gap-2">
                ...
                Sending {batchCount} Transaction{batchCount > 1 ? 's' : ''}...
              </span>
            ) : batchCount > 1
